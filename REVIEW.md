...
- Escalation keywords: src/policy.py, pinned by tests/test_policy.py (includes the sue-in-issue regression); mirror in RE.risk plus the dead-end check in web/api/pipeline.js.
...
- Offline checks (no API key needed): pytest -q (13 tests).
...
- Binding caps here: openai/gpt-oss-120b 30 RPM / 1,000 RPD / 8,000 TPM / 200,000 TPD; qwen 27B-class ~1,000 RPD; llama-3.1-8b-instant 14,400 RPD (the workhorse). You hit whichever axis arrives first.
